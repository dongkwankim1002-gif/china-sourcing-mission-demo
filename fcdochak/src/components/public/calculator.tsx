...
import type { Segment } from '@/lib/money/segments';
import { DEFAULT_INPUT, type CalcInput } from '@/lib/calc-defaults';

export interface QuoteResponse {
  count: number;
  excluded: number;
  top: { name: string; slug: string; status: string; logo: string | null; total: number; perUnit: number; mode: string; transit: [number, number]; filled: number; related: boolean }[];
  bar: { segment: Segment; amount: number | null; certainty: BarSegment['certainty']; filled: boolean }[] | null;
      ...
      <div className="min-w-0">
        ...
        <section aria-live="polite" aria-busy={loading} className={cn('mt-6 transition-opacity', loading && 'opacity-60')}>
          <div className="flex flex-wrap items-end justify-between gap-x-6 gap-y-2">
            <div>
              <p className="text-sm font-semibold text-on-ink-muted">FC 도착 총액 · 공개 요금 중 가장 낮은 곳</p>
          ...
          {bar ? (
            <div className="on-ink mt-5">
              <NineBar
                ...
                size="hero"
                ticks
                label={`가장 낮은 곳의 9구간 비중${data?.barUnit === 'won' ? '' : '(가입하면 구간 금액이 보입니다)'}`}
              />
          ...
          <div className="mt-6 rounded-md border border-white/15">
            <div className="flex items-center justify-between border-b border-white/10 px-4 py-2.5">
              <h2 className="text-sm font-bold text-on-ink">상위 5곳 총액</h2>
