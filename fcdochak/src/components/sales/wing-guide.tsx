...
function Path({ parts }: { parts: string[] }) {
  return (
    <span className="mt-1.5 flex flex-wrap items-center gap-1 text-2xs" aria-label={`누를 곳: ${parts.join(' → ')}`}>
      {parts.map((p, i) => (
...
export function WingSetupGuide({ egressIps, consented, hasKey, verified, validDays, warnDays }: { egressIps: string[]; consented: boolean; hasKey: boolean; verified: boolean; validDays: number; warnDays: number }) {
  const steps: { title: string; body: React.ReactNode; extra?: React.ReactNode; state: StepState }[] = [
    ...
    {
      title: 'OPEN API 키 발급',
      body: '추가판매정보에서 OPEN API 키 발급을 누릅니다. 업체코드 · Access Key · Secret Key 세 값이 나옵니다. 권한이 열리기까지 24시간 넘게 걸릴 수 있습니다.',
    ...
    },
    {
      ...
      body: '다른 회사 프로그램에 키를 맡길 때는 그 회사 서버 IP 를 키 설정에 넣습니다. 아래 값을 그대로 적어 주세요.',
    ...
    { title: '읽는 것 · 하지 않는 것 동의', body: '「읽는 것 · 하지 않는 것」 칸을 읽고 동의합니다(관리자). FC도착은 읽기만 하고 상품·가격·주문을 바꾸지 않습니다.', state: consented ? 'done' : 'todo' },
    { title: '세 값 넣기', body: '업체코드 · Access Key · Secret Key 를 복사해 「WING 키」 칸에 넣고 발급일을 적습니다. 암호화해 보관하고 끝 4자리만 보입니다.', extra: <Fields items={['업체코드', 'Access Key', 'Secret Key', '발급일']} />, state: hasKey ? 'done' : 'todo' },
    { title: '연결 시험', body: '「연결 시험」을 누릅니다. 연동이 꺼져 있으면 「시험 모드」로 저장·암호화·동의만 확인하고 쿠팡은 부르지 않습니다.', state: verified ? 'done' : 'todo' },
    {
      ...
      <ol className="grid gap-3 p-4 md:grid-cols-2" data-testid="wing-steps">
        {steps.map((s, n) => (
          <li key={s.title} className="grid grid-cols-[1.75rem_minmax(0,1fr)] gap-2" data-state={s.state}>
            ...
            >
              {s.state === 'done' ? <Check className="size-4" aria-label="완료" /> : n + 1}
            </span>
            <div className="min-w-0">
              <p className="flex flex-wrap items-center gap-x-2 text-sm font-semibold">
                {s.title}
                {s.state === 'todo' ? <CircleDashed className="size-3.5 text-muted" aria-label="아직" /> : null}
              </p>
