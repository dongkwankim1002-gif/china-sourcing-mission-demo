...
export default async function Invoices() {
  ...
  const tc = await getTranslations('p.cols');
          ...
          <div className="max-h-[75vh] overflow-auto">
            <table className="w-full min-w-[640px] text-sm tnum">
              <thead className="sticky top-0 bg-surface-2 text-xs text-muted">
                <tr>{[tc('no'), tc('shipment'), tc('shipper'), tc('bid'), tc('invoice'), tc('dev'), tc('issued')].map((h, i) => <th key={i} scope="col" className={cn('px-3 py-2 font-semibold', i >= 3 && i <= 5 ? 'text-right' : 'text-left')}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {rows.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-t border-line-2 hover:bg-surface-2">
                      <td className="whitespace-nowrap px-3 py-2 font-semibold">{r.invoice_no}{r.version > 1 ? ` v${r.version}` : ''}</td>
                      <td className="whitespace-nowrap px-3 py-2"><Link href={`/partner/shipments/${r.shipment_id}?tab=invoice`} className="hover:underline">{r.shipment_no}</Link></td>
