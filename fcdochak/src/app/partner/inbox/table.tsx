...
export function InboxTable({ rows, hubs }: { rows: InboxRow[]; hubs: { value: string; label: string }[] }) {
  ...
  const cols: ColumnDef<InboxRow, unknown>[] = [
    { id: 'req_no', header: tc('no'), accessorKey: 'req_no', cell: ({ row }) => <Link href={`/partner/inbox/${row.original.id}`} className="font-semibold hover:underline">{row.original.req_no}</Link> },
    ...
    { id: 'traits', header: tc('traits'), accessorKey: 'traits' },
    ...
    { id: 'state', header: tc('result'), accessorKey: 'my_state', cell: ({ row }) => state(row.original) },
